...
def printWelcome():
    ...
    delayPrint("\t__ H A N G M A N __ \n",.05)
    delayPrint(gallowsString(0)+"\n")

def printGoodbye():
    delayPrint ("\nThanks for playing")

def gallowsString(count):
    if count == 0:
        return """
    +---+
    |   |
...
def printMenu():
    delayPrint("""\n**Menu**
    Enter 1 to use your own word
    Enter 2 to use a random word
    Enter 3 to exit""")
...
def menu():
    ...
    while menu_input!="1" and menu_input!="2" and menu_input!="3":
        delayPrint("Please enter 1, 2, or 3\n\n")
        printMenu()
        menu_input = input()
    ...
    if menu_input==2:
        ...
        hangman(wordFromBank())
        return 2
    if menu_input==3:
        return 3

def wordEntry():
    delayPrint("""Enter your Hangman-Word.
    The word must be at least 4 letters,
    with no numbers or special characters.""")
    keyword = input()
    while not keyword.isalpha() or len(keyword) < 4:
        delayPrint("""Please enter a word of at least 4 letters,
        with no numbers or special characters.""")
        keyword = input()
...
def wordFromBank():
    ...
    while len(keyword) < 4:
        keyword = WORDLIST[np.random.randint(0,listsize)]
    return keyword.lower()
...
def hangman(keyword):
    def getLetter():
        while True:
            ...
            letter = input()
            if len(letter)!=1:
                delayPrint("Must be a single letter.")
                continue
            if not letter.isalpha():
                delayPrint("Must be a letter.")
                continue
            if letter in guessedlist:
                delayPrint("You've already guessed that letter.")
                continue
            break
        return letter

    def fillguess(guessword, letter):
        correct = 0
        for i,l in enumerate(keyword):
            if l == letter:
                guessword = guessword[:2*i] + l.upper() + guessword[2*i+1:]
                correct+=1
        return guessword,correct


    #delayPrint("Hangman game with the word \"{}\"".format(keyword))

    incorrect = 0
    guessedlist = set()
    guessword = "_ "*len(keyword)
    correct = 0
    while True:
        delayPrint(gallowsString(incorrect))
        delayPrint(guessword)
        print("Guessed:", end=" ")
        for let in guessedlist:
            print(let, end=" ")

        letter = getLetter()
        guessedlist.add(letter)
        delayPrint("Checking...", .02*(incorrect+1))
        if letter.lower() in keyword:
            guessword,correct_letters = fillguess(guessword, letter)
            correct+=correct_letters
        else:
            time.sleep(.2*incorrect)
            delayPrint("\n\n'{}' is not in the word".format(letter.upper()))
            incorrect+=1

        if incorrect ==6:
            delayPrint(gallowsString(incorrect))
            delayPrint("You lost")
            delayPrint("The word was '" + keyword + "'")
            break
        if correct == len(keyword):
            delayPrint("Congratulations! You guess the correct word, {}!".format(keyword.upper()))
            break
...
if __name__ == "__main__":
    if True:
        ...
        while True and choice!=3:
            ...
            delayPrint("Please enter y or n")
            playagain = input()
